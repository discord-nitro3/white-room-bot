...
async def gw_create(ctx):
    ...
    try:
        ...
        if seconds is None: return await ctx.send("❌ Invalid time format. Action aborted.")

        q2 = await ctx.send("👥 **[2/3]** Enter number of winners (e.g., `1`, `3`):")
        msg2 = await bot.wait_for('message', check=check, timeout=30.0)
        if not msg2.content.isdigit() or int(msg2.content) <= 0: return await ctx.send("❌ Invalid winners count. Action aborted.")
        ...
        return await ctx.send("❌ Setup timed out. Framework aborted.")
